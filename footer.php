<footer class="container">
    <div class="row mt-4 pb-1">
        <div class="col-12 col-sm-3 text-sm-start wrapper">
            <a href="https://github.com/" target="_blank">
                <div class="icon">
                    <i class="fa-brands fa-github  "></i>
                </div>
            </a>
            <a href="https://www.linkedin.com/" target="_blank">
                <div class="icon">
                    <i class="fa-brands fa-linkedin  "></i>
                </div>
            </a>
        </div>
        <div class="col-12 col-sm-3  pt-3 m-sm-3 p-sm-0">
            Sponsor: <a href="https://depcsuite.com" target="_blank">DePC</a>
        </div>
        <div class="col-12 col-sm-3   pt-3 m-sm-3 p-sm-0 mail-contacto">
            <a href="mailto:juanimckenna@gmail.com" target="_blank">juanimckenna@gmail.com</a>
        </div>
    </div>
</footer>
<div class="whatsapp col-12 col-sm-3">
    <a href="https://wa.me/5491150571913?text=hola!%20Quisiera%20contactar" target="_blank" title="whatapps">
        <i class="fa-brands fa-whatsapp"></i>
    </a>

</div>